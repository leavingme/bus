...
%>
        <tr>
          <td><%=(bus("bus_id"))%></td>
          <td><a href="../search.asp?q=<%=(bus("bus_name"))%>"><%=(bus("bus_name"))%></a></td>
          <td>
            <%
...
%>
		    <a href="?admin=member&amp;member=<%=(member("mbr_id"))%>"><%=(member("mbr_name"))%></a>
            <%
member.Close
Set member = Nothing
End If
...
While (NOT line.EOF)
%>
            <% If line_count > 0 Then %>- <% End If %><a href="?admin=station&amp;station=<%=(line("stn_id"))%>"><%=(line("stn_name"))%></a>
            <%
...
%>
          ...
          <td><a href="#">删除</a></td>
          <td><a href="?admin=bus&amp;bus=<%=(bus("bus_id"))%>">修改</a></td>
        </tr>
        <%
bus.MoveNext()
Wend
...
%>
        <tr>
          ...
          <td class="station"><a href="../search.asp?q=<%=Server.URLEncode(station("stn_name"))%>"><%=(station("stn_name"))%></a></td>
...
While (NOT line.EOF)
%>
            <a href="?admin=bus&amp;bus=<%=(line("bus_id"))%>"><%=(line("bus_name"))%></a>
            <%
line.MoveNext()
Wend
